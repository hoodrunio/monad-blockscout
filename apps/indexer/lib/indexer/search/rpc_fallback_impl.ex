defmodule Indexer.Search.RPCFallbackImpl do
  @moduledoc """
  RPC fallback implementation for search using OnDemand fetchers.

  This module implements the Explorer.Chain.Search.RPCFallback behaviour,
  providing actual RPC fetching capabilities using the existing OnDemand
  fetcher infrastructure.
  """

  @behaviour Explorer.Chain.Search.RPCFallback
  ...
  def fetch_address(address_hash) do
    ...
    with {:ok, balance_result} <-
      ...
      case save_address_to_db(address_hash, balance, code) do
        ...
        {:error, reason} ->
          Logger.warning("RPCFallbackImpl: Failed to save address #{address_hash_string}: #{inspect(reason)}")
          # Still return result even if DB save fails
          [build_address_search_result_from_rpc(address_hash, balance, code)]
      end
    else
      {:error, reason} ->
        Logger.debug("RPCFallbackImpl: Failed to fetch address #{address_hash_string}: #{inspect(reason)}")
        []
    end
  end
  ...
  def fetch_transaction(tx_hash) do
    case TransactionOnDemand.fetch_by_hash(nil, tx_hash) do
      ...
      {:error, reason} ->
        Logger.debug("RPCFallbackImpl: Failed to fetch transaction #{tx_hash}: #{inspect(reason)}")
        []
    end
  end
  ...
  def fetch_block_by_hash(block_hash) do
    case BlockOnDemand.fetch_by_hash(nil, block_hash) do
      ...
      {:error, reason} ->
        Logger.debug("RPCFallbackImpl: Failed to fetch block by hash #{block_hash}: #{inspect(reason)}")
        []
    end
  end
  ...
  def fetch_block_by_number(block_number) do
    case BlockOnDemand.fetch_by_number(nil, block_number) do
      ...
      {:error, reason} ->
        Logger.debug("RPCFallbackImpl: Failed to fetch block by number #{block_number}: #{inspect(reason)}")
        []
    end
  end

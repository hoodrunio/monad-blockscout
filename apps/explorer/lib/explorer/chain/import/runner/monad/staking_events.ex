defmodule Explorer.Chain.Import.Runner.Monad.StakingEvents do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.Monad.StakingEvent.t/0`.
  """
  ...
  @impl Import.Runner
  def option_key, do: :monad_staking_events
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_monad_staking_events, fn repo, _ ->
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = _options) when is_list(changes_list) do
    # Enforce StakingEvent ShareLocks order (see docs: sharelock.md)
